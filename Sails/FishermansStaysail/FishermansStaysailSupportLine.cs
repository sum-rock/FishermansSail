...
namespace FishermansSail.Sails.FishermansStaysail
{
    ...
    internal sealed class FishermansStaysailSupportLine : MonoBehaviour
    {
        public LineRenderer[] UpperSheets;
        public RopeEffect[] NativeSheets;
        private readonly Vector3[] upperPoints = new Vector3[33];

        internal static FishermansStaysailSupportLine Create(
            Transform parent,
            RopeEffect left,
            RopeEffect right
        )
        {
            var root = new GameObject("FishermansStaysail upper sheets");
            root.transform.SetParent(parent, false);
            var route = root.AddComponent<FishermansStaysailSupportLine>();
            route.NativeSheets = new[] { left, right };
            route.UpperSheets = new[]
            {
                CreateRenderer(
                    root.transform,
                    "FishermansStaysail Port upper sheet",
                    left,
                    route.upperPoints.Length
                ),
                CreateRenderer(
                    root.transform,
                    "FishermansStaysail Starboard upper sheet",
                    right,
                    route.upperPoints.Length
                ),
            };
        ...
        internal void Draw(Vector3 head, Vector3 aftGuide)
        {
            for (int side = 0; side < UpperSheets.Length; side++)
            {
                ...
                float slack = FishermansStaysailFrameGeometry.SheetSlack(
                    source.currentRopeLength,
                    source.totalRopeLength
                );
                for (int i = 0; i < upperPoints.Length; i++)
                    upperPoints[i] = FishermansStaysailFrameGeometry.UpperSheetPoint(
                        head,
                        aftGuide,
