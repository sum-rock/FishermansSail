...
internal static class Program
{
    private static void Main(string[] args)
    {
        ...
        var vertices = new[]
        {
            new Vector3(0, 0, 0), new Vector3(0, 0, 14), new Vector3(7, 0, 7),
            new Vector3(3.5f, 0, 3.5f), new Vector3(3.5f, 0, 10.5f),
            new Vector3(2, 0, 7), new Vector3(0, 0, 7)
        };
        var constraints = Constraints(new[] { 0f, 0f, float.MaxValue, float.MaxValue, float.MaxValue, 0f, 0f });
        var output = CheckInvariants(vertices, constraints);
        Require(output[3].x > vertices[3].x && output[4].x > vertices[4].x, "Both free edges must visibly widen.");
        ...
        ExpectFailure(() => PrototypeGeometry.Deform(new Vector3[3], new ClothSkinningCoefficient[3]));
        var invalid = (Vector3[])vertices.Clone();
        invalid[1].x = float.NaN;
        ExpectFailure(() => PrototypeGeometry.Deform(invalid, constraints));
        Console.WriteLine("PASS: mesh isolation, pinned vertices, corner positions, visible deformation, bounds, and invalid inputs.");
        ...
        if (args.Length > 0)
        {
            ...
            var source = root.GetProperty("vertices").EnumerateArray()
                .Select(v => new Vector3(v[0].GetSingle(), v[1].GetSingle(), v[2].GetSingle())).ToArray();
            var coefficients = Constraints(root.GetProperty("maxDistances").EnumerateArray().Select(v => v.GetSingle()).ToArray());
            var changed = CheckInvariants(source, coefficients);
            double before = 0, after = 0;
            foreach (var triangle in root.GetProperty("triangles").EnumerateArray())
            {
                int a = triangle[0].GetInt32(), b = triangle[1].GetInt32(), c = triangle[2].GetInt32();
            ...
            Console.WriteLine($"PASS: actual brig mesh ({source.Length} vertices, {coefficients.Count(c => c.maxDistance == 0)} pinned); " +
                $"no inverted triangles; projected area {before:F2} -> {after:F2}.");
        }
    }

    private static Vector3[] CheckInvariants(Vector3[] source, ClothSkinningCoefficient[] constraints)
    {
        ...
        Require(!ReferenceEquals(source, output) && source.Length == output.Length, "Return a separate array with the original topology.");
        float minX = source.Min(v => v.x), maxX = source.Max(v => v.x);
        bool anyChanged = false;
        for (int i = 0; i < source.Length; i++)
        {
            Require(Same(source[i], snapshot[i]), "The input geometry was mutated.");
            Require(constraints[i].maxDistance == originalConstraints[i], "The cloth constraints were mutated.");
            Require(output[i].y == source[i].y && output[i].z == source[i].z, "Deformation must stay in the existing cloth plane.");
            Require(float.IsFinite(output[i].x) && output[i].x >= minX && output[i].x <= maxX, "The deformation escaped its original bounds.");
