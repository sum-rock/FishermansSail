...
internal static class FixedHeadChecks
{
    internal static void Run()
    {
        ...
        )
        {
            ...
            CutChecks.Near(head.x, c[1].x, width * 1e-5f, "fixed head height");
            CutChecks.Near(
                (head - c[0]).magnitude,
                (c[1] - c[0]).magnitude,
                width * 1e-5f,
            ...
            for (int sheet = -40; sheet <= 40; sheet += 5)
            {
                ...
                clew = FishermansStaysailFrameGeometry.RotateAroundMast(
                    ...
                    c[0],
                    Vector3.right,
                    FishermansStaysailFixedHead.LowerAngle(sheet, side * 14, unroll)
                );
        ...
        Console.WriteLine(
            "PASS: Mk.A 14-to-zero proportional upper reef angle, independent lower-sheet sweep, tack transitions and edge budgets."
        );
    }
    ...
    private static void CheckTackState()
    {
        ...
        foreach (int desired in new[] { 1, -1, 1, -1 })
        {
            ...
            CutChecks.Near(state.Side, desired, 0, "tack settles at exact fixed angle");
        }
        ...
        CutChecks.Near(moved.Side, -1, 0, "heel/rake cannot change tack selection");
