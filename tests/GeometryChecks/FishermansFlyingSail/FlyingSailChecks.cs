...
internal static class FlyingSailChecks
{
        ...
        foreach (float radius in new[] { 0.1f, 0.4f, 0.8f })
        {
            ...
            foreach (float angle in new[] { -70f, -25f, 0f, 25f, 70f })
            {
                var posed = rest.Select(c =>
                        FishermansFlyingSailFrameGeometry.RotateAroundMast(c, pivot, axis, angle)
                    )
                    .ToArray();
                ...
                {
                    var upper = FishermansFlyingSailFrameGeometry.UpperHead(
                        rest[1],
                        posed[1],
                        pivot,
                        axis,
                    ...
                    var reefed = FishermansFlyingSailMastInstallationGeometry.HoistCorner(
                        rest[2],
                        rest[0],
                        rest[2] - axis * width,
                        unroll
    ...
    private static void CheckUpperGuideSelection()
    {
        ...
        {
            var up = FishermansFlyingSailFrameGeometry.RotateAroundMast(
                Vector3.up,
                Vector3.zero,
                Vector3.forward,
                heel
            ...
            var posed = guides
                ...
                .ToArray();
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(ActiveGuides(posed), up) == 1,
            ...
            var candidates = new[]
            {
                new FishermansFlyingSailSheetGuideState(posed[1], false, false),
                new FishermansFlyingSailSheetGuideState(posed[3], true, true),
                new FishermansFlyingSailSheetGuideState(posed[1], false, false),
            };
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == 1,
                "Absent topmast stole the route from the active lower-mast pulley."
            );
            candidates[0] = new FishermansFlyingSailSheetGuideState(posed[1], true, true);
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == 0,
                "Enabling the topmast did not select its higher pulley."
            );
            candidates[0] = new FishermansFlyingSailSheetGuideState(posed[1], true, false);
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == 1,
                "An inactive pulley child must not receive the control line."
            );
            candidates[0] = new FishermansFlyingSailSheetGuideState(posed[1], false, true);
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == 1,
                "An active referenced guide cannot make an inactive owning mast eligible."
            );
            candidates[1] = new FishermansFlyingSailSheetGuideState(posed[3], false, false);
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == -1,
                "Loading with every mast inactive must leave no pulley selected."
            );
            candidates[1] = new FishermansFlyingSailSheetGuideState(posed[3], true, true);
            Check(
                FishermansFlyingSailFrameGeometry.HighestGuideIndex(candidates, up) == 1,
        ...
        Check(
            FishermansFlyingSailFrameGeometry.HighestGuideIndex(
                Array.Empty<FishermansFlyingSailSheetGuideState>(),
                Vector3.up
            ) == -1,
        ...
        Check(
            FishermansFlyingSailFrameGeometry.HighestGuideIndex(ActiveGuides(invalid), Vector3.up)
                == -1
                && FishermansFlyingSailFrameGeometry.HighestGuideIndex(
    ...
    private static FishermansFlyingSailSheetGuideState[] ActiveGuides(Vector3[] positions) =>
        positions
    ...
    private static void CheckUpperSheets()
    {
        ...
        Check(
            FishermansFlyingSailFrameGeometry.SheetSlack(1, 1) == 0
                ...
                && FishermansFlyingSailFrameGeometry.SheetSlack(1, 2) == 0
                && FishermansFlyingSailFrameGeometry.SheetSlack(0, 0) == 0
                && FishermansFlyingSailFrameGeometry.SheetSlack(float.NaN, 1) == 0,
